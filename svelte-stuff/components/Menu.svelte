<script lang="ts">
    ...
    import type { GifStory, Story, TextStory } from "../shared/types";

    export let storyType: string;
    export let tsvscode: any;
    export let textStory: TextStory;
    export let gifStory: GifStory;
    export let isFriend: boolean = false;
    export let creatorUsername: string;
...
<div class="menu">
    {#if !story.hasLiked && likeClickable}
        <svg
            on:click={async () => {
                likeClickable = false;
                try {
    ...
    {:else if likeClickable}
        ...
        >
    {:else if !story.hasLiked && !likeClickable}
        <svg viewBox="0 0 24 24" fill="#fff" class="heart"
            ><g>
                <path
                    d="M12 21.638h-.014C9.403 21.59 1.95 14.856 1.95 8.478c0-3.064 2.525-5.754 5.403-5.754 2.29 0 3.83 1.58 4.646 2.73.814-1.148 2.354-2.73 4.645-2.73 2.88 0 5.404 2.69 5.404 5.755 0 6.376-7.454 13.11-10.037 13.157H12zM7.354 4.225c-2.08 0-3.903 1.988-3.903 4.255 0 5.74 7.034 11.596 8.55 11.658 1.518-.062 8.55-5.917 8.55-11.658 0-2.267-1.823-4.255-3.903-4.255-2.528 0-3.94 2.936-3.952 2.965-.23.562-1.156.562-1.387 0-.014-.03-1.425-2.965-3.954-2.965z"
                />
        ...
        >
    {:else if story.hasLiked && !likeClickable}
        <svg viewBox="0 0 24 24" fill="rgb(224, 36, 94)" class="heart"
            ><g>
                <path
                    d="M12 21.638h-.014C9.403 21.59 1.95 14.856 1.95 8.478c0-3.064 2.525-5.754 5.403-5.754 2.29 0 3.83 1.58 4.646 2.73.814-1.148 2.354-2.73 4.645-2.73 2.88 0 5.404 2.69 5.404 5.755 0 6.376-7.454 13.11-10.037 13.157H12z"
                />
    ...
    {#if currentUserId === story.creatorId || currentUserId === "dac7eb0f-808b-4842-b193-5d68cc082609"}
        <svg
            on:click={async () => {
                try {
                    if (storyType === "gif") {
        ...
        >
    {/if}
    {#if !isFriend && currentUserId !== story.creatorId}
        <svg
            on:click={async () => {
                try {
                    await mutation(`/add-friend/${creatorUsername}`, {});
                    const friendsData = await query(
                        `/is-friend/${creatorUsername}`
                    );
                    if (friendsData.ok === true) {
                        ...
                        tsvscode.postMessage({
                            type: "onError",
                            value:
                                "Something went wrong! This user might have blocked you.",
                        });
                    }
                } catch {}
            }}
        ...
        >
    {:else if currentUserId !== story.creatorId}
        <svg
            on:click={async () => {
                await mutation(`/remove-friend/${creatorUsername}`, {});
                isFriend = false;
            }}
            viewBox="0 0 24 24"
            width="18"
